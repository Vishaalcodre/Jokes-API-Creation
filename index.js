...
app.get("/filter", (req, res) => {
  ...
  const jokeFound = jokes.filter((joke) => joke.jokeType === jokeType);
  res.json(jokeFound);
});
//4. POST a new joke
...
app.put("/jokes/:id", (req, res) => {
  ...
  const jokeFound = jokes.findIndex((joke) => joke.id === jokeId);
  jokes[jokeFound] = newJoke;
  console.log(newJoke);
  res.json(newJoke);
});
//6. PATCH a joke
...
app.delete("/jokes/all", (req, res) => {
  ...
  if (userApiKey === masterKey) {
    ...
    res
    .status(404)
    .json({error: "Sorry you are not authorised to delete"});
  }
});
